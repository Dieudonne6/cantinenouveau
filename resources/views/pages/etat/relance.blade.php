  ...
  <div class="printable">
    ...
    <div class="page">
      ...
      <div class="relance">
        ...
        <div class="content">
          <p>Chers parents,</p><br>
          <p>Sauf erreur ou omission de notre part, nous rappelons qu'au titre des frais de
            cantine <strong> {{ $params->anneencours_paramcontrat }}-{{ $params->anneencours_paramcontrat + 1 }} </strong> de votre enfant, vous restez devoir dans nos livres, la somme de  <strong> {{ $result['total_du'] }} Fcfa</strong>  
            {{-- cantine <strong> {{ $databaseName }} </strong> de votre enfant, vous restez devoir dans nos livres, la somme de {{ $result['total_du'] }} --}}
            pour le compte de <strong> {{ implode(', ', $result['mois_impayes']) }}</strong>.</p>
            <p>Vous êtes invités à régler ce solde au plus tard le 
              <strong> <?php echo \Carbon\Carbon::parse($result['datebuttoire'])->translatedFormat('l j F Y'); ?></strong></p>
